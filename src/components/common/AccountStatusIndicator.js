...
const InteractiveBrokersStatusIndicator = ({ account }) => {
    ...
    console.log('IB Status:', { doStatus, ibeamAuthenticated, account });
    ...
    const getStatusConfig = () => {
        switch (doStatus) {
            ...
            case 'running':
                // Only show green if server is running AND IBEam is authenticated
                return {
                    color: ibeamAuthenticated === true ? 'green.400' : 'blue.400',
                    message: ibeamAuthenticated === true 
                        ? 'Server is running and IBEam is authenticated' 
                        : 'Server is running, IBEam connecting...',
                    animate: ibeamAuthenticated !== true
                };
            case 'stopping':
                return {
                    color: 'orange.400',
